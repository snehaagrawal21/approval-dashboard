...
This one file *is* the whole dashboard — the filters, the headline numbers, the flagged-issue cards, the category breakdown, the organisation ranking, the full request list, all of it, built right in. Near the bottom of the file sits a block holding every request's data. That block is the *only* part that changes day to day — the look, the flag rules, the click behavior never change on their own.
